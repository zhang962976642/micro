...
const path = require("path"),
  { controllersPath, servicesPath } = require("./config/config");
...
class MicroLoad {
    ...
    for(let name in controllers){
      ...
      for(let prop of props){
        ...
        app.controllers[name][prop] = (req, res, next) => {
          let currentClass = new controllers[name]();
          return currentClass[prop]({req,res,next, services: app.services || {}});
        };
  ...
  loadServices(app){
    ...
    for(let name in services){
      ...
      if(app.services[name] === undefined){
